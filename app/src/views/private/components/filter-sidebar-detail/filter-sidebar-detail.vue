<template>
	<sidebar-detail :badge="filters.length > 0 ? filters.length : null" icon="filter_list" :title="$t('advanced_filter')">
		...
		<d-menu attached :disabled="loading">
			<template #activator="{ toggle, active }">
				<d-input @click="toggle" :class="{ active }" readonly :value="$t('add_filter')" :disabled="loading">
					<template #prepend><d-icon fa="plus-circle" /></template>
					<template #append><d-icon fa="chevron-down" /></template>
				</d-input>
			</template>
		...
		<template v-if="showArchiveToggle">
			...
			<d-checkbox v-model="archived" :label="$t('show_archived_items')"></d-checkbox>
		</template>
	</sidebar-detail>
</template>

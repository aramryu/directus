<template>
	<d-notice v-if="!collectionField" type="warning">
		{{ $t('interfaces.display-template.collection_field_not_setup') }}
	</d-notice>
	<d-notice v-else-if="collection === null" type="warning">
		{{ $t('interfaces.display-template.select_a_collection') }}
	</d-notice>
	<d-field-template v-else :collection="collection" @input="$listeners.input" :value="value" :disabled="disabled" />
</template>

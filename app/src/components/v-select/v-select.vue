	...
	>
		<template #activator="{ toggle, active }">
			<div v-if="inline" class="inline-display" :class="{ placeholder: !displayValue }" @click="toggle">
				...
				<d-icon name="expand_more" :class="{ active }" />
			</div>
			<d-input
				v-else
			...
			>
				...
				<template #append><d-icon name="expand_more" :class="{ active }" /></template>
			</d-input>
		</template>
		...
		<d-list class="list">
			...
			<template v-if="allowOther && multiple === true">
				...
				>
					...
					<d-list-item-icon>
						<d-icon name="close" @click="setOtherValue(otherValue.key, null)" />
					</d-list-item-icon>
				</d-list-item>

<template>
	<div class="label type-label" :class="{ disabled: disabled }">
		...
		<span @click="toggle">
			{{ field.name }}
			<d-icon class="required" sup name="star" v-if="field.schema && field.schema.is_nullable === false" />
			<d-icon v-if="!disabled" class="ctx-arrow" :class="{ active }" name="arrow_drop_down" />
		</span>
	</div>
</template>

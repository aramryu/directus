			...
			<div class="field">
				...
				<draggable v-model="activeFields" handle=".drag-handle" :set-data="hideDragImage">
					...
					>
						<template #append>
							<div class="spacer" />
							<d-icon @click.stop fa="grip-lines" class="drag-handle" />
						</template>
					</d-checkbox>
				</draggable>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		function useTable() {
			...
			function toggleCheckbox(item: Item) {
				if (_selection.value?.includes(item[primaryKeyField.value.field]))
				{
					_selection.value.splice(item[primaryKeyField.value.field].index, 1);
				} else {
					_selection.value.push(item[primaryKeyField.value.field]);
				}
			};

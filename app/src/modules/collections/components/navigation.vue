<template>
	<d-list large class="collections-navigation" @contextmenu.native.prevent.stop="$refs.contextMenu.activate">
		<template v-if="customNavItems && customNavItems.length > 0">
			<template v-for="(group, index) in customNavItems">
				...
				>
					<d-list-item :exact="exact" v-for="navItem in group.items" :key="navItem.to" :to="navItem.to">
						<d-list-item-icon><d-icon :name="navItem.icon" /></d-list-item-icon>
						<d-list-item-content>
							<d-text-overflow :text="navItem.name" />
						</d-list-item-content>
					</d-list-item>
				</template>
				<template v-else>
		...
		<d-list-item v-else :exact="exact" v-for="navItem in navItems" :key="navItem.to" :to="navItem.to">
			<d-list-item-icon><d-icon :name="navItem.icon" /></d-list-item-icon>
			<d-list-item-content>
				<d-text-overflow :text="navItem.name" />
			</d-list-item-content>
		...
		<div v-if="!customNavItems && !navItems.length && !bookmarks.length" class="empty">
			<template v-if="isAdmin">
				<d-button fullWidth outlined dashed to="/settings/data-model/+">{{ $t('create_collection') }}</d-button>
			</template>
			<template v-else>
		...
		<d-menu ref="contextMenu" show-arrow placement="bottom-start">
			<d-list>
				<d-list-item @click="hiddenShown = !hiddenShown">
					<d-list-item-icon>
						<d-icon :name="hiddenShown ? 'visibility_off' : 'visibility'" />
					</d-list-item-icon>
					<d-list-item-content>
						<d-text-overflow :text="hiddenShown ? $t('hide_hidden_collections') : $t('show_hidden_collections')" />
					</d-list-item-content>
				</d-list-item>
			</d-list>
		</d-menu>

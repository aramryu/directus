...
function mergePermissions(...permissions) {
    const allPermissions = lodash_1.flatten(permissions);
    ...
    const result = Array.from(mergedPermissions).map((perm) => {
        return lodash_1.omit(perm, ['id', 'system']);
    });
    return result;
}
...
function mergePerm(currentPerm, newPerm) {
    ...
    if (newPerm.presets) {
        presets = lodash_1.merge({}, presets, newPerm.presets);
    }
    if (newPerm.limit && newPerm.limit > (currentPerm.limit || 0)) {
        limit = newPerm.limit;
    }
    return Object.assign(Object.assign({}, currentPerm), { permissions,
        validation,
        fields,
        presets,
        limit });
